...
- Source-review ledger records now require a credential-free HTTPS URL with a
  valid port that already appears in the reviewed rule's `references:`. The
  current corpus has 891 HTTPS references and no plain-HTTP references; review
  outcomes remain absent and therefore explicitly `not_assessed`, rather than
  being inferred from URL shape.
